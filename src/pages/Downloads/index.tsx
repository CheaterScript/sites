...
const WindowsImageIcon: any = new URL(
  "assets/icon_windows_64.png",
  import.meta.url
);
const LinuxImageIcon: any = new URL("assets/icon_linux.png", import.meta.url);
const GooglePlayImageIcon: any = new URL(
  "assets/icon_google_play.png",
  import.meta.url
);
...
export function WrapGlobalDownloadButton(props: any = {}) {
  ...
  const rightIconFn = isMacOS
    ? (props: any = {}) => (
        <CaretDown className={"ml-1 opacity-60"} {...props} />
      )
    : isIOS
    ? (props: any = {}) =>
        appState.sm.get() ? (
  ...
  useEffect(() => {
    releases.some((it) => {
      if (os[it?.[0].toString().toLowerCase()]) {
        setActive(it);
        return true;
      }
    });
  ...
  const subItems = isMacOS ? (
    <div className="sub-items flex flex-col absolute top-5 right-0 w-full pt-6">
      <div className="sub-items-inner">
        <a
          className={"block"}
          href={resolveDownloadHref(appState, active, isIOS, "macos-x64")}
        >
          <div className="flex items-center">
            ...
            <div className={"w-full flex flex-col opacity-80"}>
              <span className="text-sm">Intel CPU</span>
              <span className="text-[11px] opacity-60">适用于大多数Mac</span>
            </div>
          </div>
        </a>
        ...
        <a
          className={"block"}
          href={resolveDownloadHref(appState, active, isIOS, "macos-arm64")}
        >
          <div className="flex items-center">
            <div className="flex pr-2">
            ...
            <div className={"w-full flex flex-col opacity-80"}>
              <span className="text-sm">Apple CPU</span>
    ...
    <span className="sub-items absolute top-6 right-2 z-10 flex justify-center translate-x-4 transition-opacity qr">
  ...
  return (
    ...
    >
      <a
        className={"block"}
        target={"_blank"}
        href={resolveDownloadHref(appState, active, isIOS)}
      >
        {children({
          active,
          leftIconFn:
            typeof activePlatformIcon === "function"
              ? activePlatformIcon
              : () => activePlatformIcon,
          rightIconFn,
        })}
...
export function HeadDownloadLinksSelect(props: {
  activeRelease: any;
  setActiveRelease: any;
}) {
  ...
  return (
    <div className={cx("selects", `active-of-${activeLabel?.toLowerCase()}`)}>
      <div className="app-form-select-wrap">
        <span className="icon">{activeIcon}</span>
...
export function HeadDownloadLinksTabs(props: {
  activeRelease: any;
  setActiveRelease: any;
}) {
  const { activeRelease, setActiveRelease } = props;
  ...
  const resolvePanel = function ([label, icon]: [string, any]) {
    ...
    switch (label) {
      case "iOS":
        return (
          <div
            className="w-full sm:w-auto flex flex-col items-center is-ios cursor-crosshair"
            id={label}
            role="tabpanel"
          ...
          >
            <Button
              ...
              rightIcon={
                isSm ? (
                  <DownloadSimple className="opacity-50" />
                ) : (
      ...
      case "MacOS":
        return (
          <div
            className="w-full sm:w-auto sm:flex sm:space-x-6"
            id={label}
            role="tabpanel"
            aria-labelledby={"tab-" + label}
          >
            <div className="flex flex-col items-center">
              <Button
                ...
                leftIcon={
                  <IntelIcon
                    className={"w-6 h-6 sm:w-8 sm:h-8 bg-gray-900"}
                    color={"white"}
                  />
                }
              ...
              <span className="text-xs opacity-60 py-2">适用于大多数Mac</span>
            </div>
    ...
    <div className="app-head-downloads">
      <div className="flex flex-col pt-10 sm:items-center sm:pt-20 pb-12 text-slogan">
        <h1 className="text-3xl sm:text-6xl">
          <strong className="font-semibold tracking-wide">下载 </strong>
          <span className="text-logseq-50/80">Logseq 应用。</span>
        </h1>

        <h2 className="text-xl flex flex-col justify-center sm:items-center pt-2 tracking-wide">
